...
trait ManageImages
{
    ...
    {
        $imageType = 'cv-images.' . $imageTypeKey;
    ...
    private function storeImage($propsImage, $request)
    {
        ...
        $spatie->fit(
            ...
            $propsImage['width'],
            $propsImage['height']
        )->save(public_path('/') . $propsImage['temp'] . $temp_image_name);
    ...
    {
        if (file_exists(public_path('/') . $propsImage['temp'] . $request->fileName)){

            File::delete(public_path('/') . $propsImage['temp'] . $request->fileName);
